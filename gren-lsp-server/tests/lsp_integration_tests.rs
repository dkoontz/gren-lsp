use gren_lsp_server::test_utils::{LspTestClient, create_test_gren_source};
use lsp_types::*;
...
async fn test_lsp_test_utilities() {
    ...
    assert_eq!(init_params.client_info.as_ref().unwrap().name, "test-client");
    ...
    assert_eq!(did_open_params.text_document.uri.as_str(), "file:///test.gren");
    ...
    assert_eq!(hover_params.text_document_position_params.position.character, 8);
    
    // Test goto definition request creation
    let goto_def_params = LspTestClient::create_goto_definition_request("file:///test.gren", 15, 10);
    assert_eq!(goto_def_params.text_document_position_params.position.line, 15);
    assert_eq!(goto_def_params.text_document_position_params.position.character, 10);
}
...
async fn test_document_change_notification() {
    ...
    let did_change_params = LspTestClient::create_did_change_notification(
        "file:///test.gren",
        2,
        changes,
    );
    
    assert_eq!(did_change_params.text_document.uri.as_str(), "file:///test.gren");
    assert_eq!(did_change_params.text_document.version, 2);
    assert_eq!(did_change_params.content_changes.len(), 1);
    assert_eq!(did_change_params.content_changes[0].text, "module UpdatedModule");
}
