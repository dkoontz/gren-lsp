...
type Direction = North | South | East | West"#;
    ...
    match handlers.document_symbols(params).await {
        Ok(Some(DocumentSymbolResponse::Nested(symbols))) => {
            println!(
                "✅ Document symbols using tree-sitter structure ({}): ",
                symbols.len()
            );
            ...
            if let Some(color) = symbols.iter().find(|s| s.name == "Color") {
                ...
                if constructors == expected {
                    ...
                    println!(
                        "❌ Color type incorrect: expected {:?}, got {:?}",
            ...
            if let Some(result) = symbols.iter().find(|s| s.name == "Result") {
                ...
                if constructors == expected {
                    ...
                    println!(
                        "❌ Result type incorrect: expected {:?}, got {:?}",
                        expected, constructors
                    );
                }
            }
            ...
            if let Some(direction) = symbols.iter().find(|s| s.name == "Direction") {
                let empty_vec = vec![];
                let constructors: Vec<_> = direction
                    .children
                    .as_ref()
                    .unwrap_or(&empty_vec)
                    .iter()
                ...
                if constructors == expected {
                    ...
                    println!(
                        "❌ Direction type incorrect: expected {:?}, got {:?}",
                        expected, constructors
                    );
                }
            }
            ...
            if let Some(point) = symbols.iter().find(|s| s.name == "Point") {
                ...
                if constructor_count == 0 {
                    ...
                    println!(
                        "❌ Point type alias incorrectly has {} constructors",
                        constructor_count
                    );
                }
            }
