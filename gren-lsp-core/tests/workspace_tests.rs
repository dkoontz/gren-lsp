...
fn test_workspace_initialization() {
    ...
    assert!(
        workspace.is_ok(),
        "Workspace should initialize successfully"
    );
...
fn test_workspace_with_capacity() {
    ...
    assert!(
        workspace.is_ok(),
        "Workspace should initialize with custom capacity"
    );
...
fn test_document_version_check() {
    ...
    assert!(
        result.is_ok(),
        "Update should not fail but should be ignored"
    );
...
fn test_update_nonexistent_document() {
    ...
    assert!(
        result.is_ok(),
        "Update of nonexistent document should not fail"
    );
}
