...
impl SymbolIndex {
    ...
    pub fn find_symbol(&self, name: &str) -> SqlResult<Vec<Symbol>> {
        ...
        let symbol_iter = stmt.query_map([format!("%{}%", name)], |row| {
            let uri = Url::parse(&row.get::<_, String>(2)?)
            ...
            Ok(Symbol {
                ...
                container_name: {
                    ...
                    if container.is_empty() { None } else { Some(container) }
                },
                type_signature: {
    ...
    pub fn clear_file_symbols(&self, file_uri: &str) -> SqlResult<()> {
        let connection = self.connection.lock().unwrap();
        connection.execute(
            "DELETE FROM symbols WHERE file_uri = ?1",
            [file_uri],
        )?;
        Ok(())
    }
...
impl SymbolExtractor {
    pub fn new() -> Result<Self> {
        ...
        let module_query = Query::new(language, r#"
            ; Module declarations
            (module_declaration 
                (upper_case_qid 
    ...
    pub fn extract_symbols(&self, tree: &Tree, source: &str, file_uri: &Url) -> Result<Vec<Symbol>> {
        let mut symbols = Vec::new();
        let mut cursor = QueryCursor::new();
        
        // Extract functions
        symbols.extend(self.extract_functions(tree, source, file_uri, &mut cursor)?);
        
        // Extract types
        symbols.extend(self.extract_types(tree, source, file_uri, &mut cursor)?);
        
        // Extract constructors
        symbols.extend(self.extract_constructors(tree, source, file_uri, &mut cursor)?);
        
        // Extract modules
        symbols.extend(self.extract_modules(tree, source, file_uri, &mut cursor)?);
        
        debug!("Extracted {} symbols from {}", symbols.len(), file_uri);
        Ok(symbols)
    }
    
    fn extract_functions(&self, tree: &Tree, source: &str, file_uri: &Url, cursor: &mut QueryCursor) -> Result<Vec<Symbol>> {
        ...
        let mut function_defs: std::collections::HashMap<String, (Range, Option<String>)> = std::collections::HashMap::new();
                    ...
                    match capture_name.as_str() {
                        ...
                        "function.type_name" => {
                            ...
                            for capture2 in m.captures {
                                let capture2_name = &self.function_query.capture_names()[capture2.index as usize];
                                if capture2_name == "function.type_sig" {
                                    if let Ok(type_text) = capture2.node.utf8_text(source_bytes) {
                                        type_annotations.insert(func_name.clone(), Self::clean_type_signature(type_text));
                                        break;
                                    }
                                }
                            }
        ...
        // Second pass: combine function definitions with their type annotations
        for (func_name, (range, _)) in function_defs {
            ...
            functions.push(Symbol {
                ...
                type_signature,
                documentation: None, // TODO: Extract documentation comments
            });
        }
    ...
    fn extract_types(&self, tree: &Tree, source: &str, file_uri: &Url, cursor: &mut QueryCursor) -> Result<Vec<Symbol>> {
    ...
    fn extract_constructors(&self, tree: &Tree, source: &str, file_uri: &Url, cursor: &mut QueryCursor) -> Result<Vec<Symbol>> {
    ...
    fn extract_modules(&self, tree: &Tree, source: &str, file_uri: &Url, cursor: &mut QueryCursor) -> Result<Vec<Symbol>> {
...
"#;
        ...
        // Parse the source
        let tree = parser.parse(gren_source).expect("Failed to parse").expect("No tree returned");
        
        // Extract symbols
        let file_uri = Url::parse("file:///utils.gren").expect("Invalid URI");
        let symbols = extractor.extract_symbols(&tree, gren_source, &file_uri)
        ...
        let types: Vec<_> = symbols.iter().filter(|s| s.kind == SymbolKind::CLASS).collect();
        let constructors: Vec<_> = symbols.iter().filter(|s| s.kind == SymbolKind::CONSTRUCTOR).collect();
        ...
        let add_func = functions.iter().find(|f| f.name == "add").expect("Should find add function");
        assert_eq!(add_func.type_signature, Some("Int -> Int -> Int".to_string()));
        
        let multiply_func = functions.iter().find(|f| f.name == "multiply").expect("Should find multiply function");
        assert_eq!(multiply_func.type_signature, Some("Float -> Float -> Float".to_string()));
        
        let distance_func = functions.iter().find(|f| f.name == "distance").expect("Should find distance function");
        assert_eq!(distance_func.type_signature, Some("Point -> Point -> Float".to_string()));
        
        // Test function without type annotation
        let simple_func = functions.iter().find(|f| f.name == "simple").expect("Should find simple function");
        assert_eq!(simple_func.type_signature, None, "simple function should have no type signature");
        
        // Test type extraction
        assert!(types.iter().any(|t| t.name == "Point"), "Should find Point type");
        assert!(types.iter().any(|t| t.name == "Shape"), "Should find Shape type");
        
        // Test constructor extraction
        assert!(constructors.iter().any(|c| c.name == "Circle"), "Should find Circle constructor");
        assert!(constructors.iter().any(|c| c.name == "Rectangle"), "Should find Rectangle constructor");
        assert!(constructors.iter().any(|c| c.name == "Triangle"), "Should find Triangle constructor");
        
        // Test module extraction
        assert!(modules.iter().any(|m| m.name == "Utils"), "Should find Utils module");
    }
...
"#;
        ...
        for symbol in &symbols {
            println!("  {} ({:?}) at {:?}", symbol.name, symbol.kind, symbol.location.range);
        ...
        // Should find module
        assert!(symbol_names.contains(&&"Main".to_string()), "Should find Main module");
        
        // Should find types
        assert!(symbol_names.contains(&&"User".to_string()), "Should find User type alias");
        assert!(symbol_names.contains(&&"Status".to_string()), "Should find Status type");
        
        // Should find constructors
        assert!(symbol_names.contains(&&"Active".to_string()), "Should find Active constructor");
        assert!(symbol_names.contains(&&"Inactive".to_string()), "Should find Inactive constructor");
        
        // Should find functions
        assert!(symbol_names.contains(&&"length".to_string()), "Should find length function");
        assert!(symbol_names.contains(&&"main".to_string()), "Should find main function");
        
        // Verify function has type signature
        let length_symbol = symbols.iter().find(|s| s.name == "length").expect("Should find length function");
        assert!(length_symbol.type_signature.is_some(), "length function should have type signature");
    }
    ...
    fn test_symbol_index_operations() {
        ...
        let found_after_clear = index.find_symbol("testFunction").expect("Failed to search after clear");
        assert_eq!(found_after_clear.len(), 0, "Should find no symbols after clearing");
    }
}